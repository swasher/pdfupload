...
При этом база данных не затрагивается. Сначала выполняется деплой на тестовый сервер, тестируется, потом деплоится на рабочий. Если во время тестирования на
тестовом сервере что-то пошло не так, то перед следующей попыткой деплоя нужно привести тестовый сервер в состояние,
идентичное рабочему (production) серверу:
